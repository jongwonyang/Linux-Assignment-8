...
unsigned long long balance_dirty_pages_ratelimited_count = 0;
unsigned long long balance_dirty_pages_ratelimited_time = 0;
...
again:
		...
		calclock(localclock, &balance_dirty_pages_ratelimited_time,
...
ssize_t __pxt4_generic_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (iocb->ki_flags & IOCB_DIRECT) {
		...
		status = pxt4_generic_perform_write(file, from, pos = iocb->ki_pos);
		/*
