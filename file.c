...
static void print_target_dm(unsigned long pid,
		const char *name,
...
DEFINE_DS_MONITORING(my_dm, get_target_idx, get_target_name, print_target_dm);
...
static ssize_t pxt4_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	find_ds_monitoring(&my_dm, current);
...
loff_t pxt4_llseek(struct file *file, loff_t offset, int whence)
{
	 ...
	 * inline data now because pxt4_dio_supported() checked for that.
	 */
